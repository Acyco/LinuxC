/* setbuf_example.c  -- 修改buf大小 写文件的实例程序 */
#include <stdio.h>
#include <error.h>
#include <string.h>
...
int main (int argc, char **argv)
{
    ...
    printf ("test setbuf(no buf) !check l_buf.txt, because line buf, \
             only data befor enter send to file\n");
    printf ("now buf data is :buf = %s\n", buf);
    printf ("press enter to continue!\n");
    getchar ();
    fclose (fp);
    ...
    memset (buf, '\0', 128);
    fwrite (msg2, sizeof (msg2), 1, fp);
    printf ("test setbuf(no buf) !check l_buf.txt\n");
    printf ("now buf data is :buf = %s\n", buf);
    printf ("press enter to continue!\n");
    getchar ();
    fclose (fp);
